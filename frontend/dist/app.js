...
const state = {
    screen: 'upload', tab: 'tasks', health: null,
    file: null, fileUrl: '', context: '', consent: false, visionMode: 'auto',
    extraction: null, result: null,
    corrections: {}, answers: {},
...
const icon = (name) => `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[name] || icons.document}</svg>`;
const esc = (value) => String(value ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
const list = (items, className = 'text-list') => `<ul class="${className}">${items.map(x => `<li>${esc(x)}</li>`).join('')}</ul>`;
const statusLabel = (task) => ({ ready: 'Ready to start', waiting: 'Waiting on dependencies', blocked: 'Needs clarification' }[task.readiness]);
...
function render() {
    ...
    document.querySelector('.new-plan').disabled = Boolean(state.busy);
...
function requirementCard(req, editable) {
...
function planView() {
    ...
    <nav class="plan-tabs" aria-label="Plan sections">${['tasks', 'requirements', 'implementation', 'questions'].map(tab => `<button data-action="tab" data-tab="${tab}" class="${state.tab === tab ? 'active' : ''}" aria-current="${state.tab === tab ? 'page' : 'false'}">${{ tasks: 'Engineering tasks', requirements: 'Source requirements', implementation: 'Implementation context', questions: 'Open questions' }[tab]}${tab === 'questions' && plan.open_questions.length ? `<span class="count">${plan.open_questions.length}</span>` : ''}</button>`).join('')}<button class="prompts-export" data-action="export-prompts">${icon('code')} Export prompts</button></nav>
...
async function api(url, init = {}) {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`The backend returned a non-JSON response (HTTP ${response.status}). Check the server and proxy settings.`);
        }
        if (!response.ok) {
        ...
        if (error instanceof DOMException && error.name === 'AbortError')
            throw new Error('The browser stopped waiting for this request. Server work may still be finishing; avoid immediately submitting duplicate runs.');
        throw error;
    }
    finally {
...
function selectFile(file) {
    ...
    if (!file.name.toLowerCase().endsWith('.pdf')) {
        state.error = 'Choose a PDF document. Images and renamed text files are not supported.';
        render();
        return;
    }
    if (!file.size || file.size > limit) {
        state.error = `Choose a non-empty PDF no larger than ${state.health?.max_file_mb ?? 20} MB.`;
        render();
        return;
    }
    if (state.fileUrl)
...
async function handleAction(action, element) {
    ...
    switch (action) {
        case 'choose':
            ...
            break;
        case 'new':
            if ((state.extraction || state.file) && !window.confirm('Start a new plan? The current workspace is not saved. Export anything you need first.'))
                return;
            if (state.fileUrl)
                URL.revokeObjectURL(state.fileUrl);
        ...
        case 'navigate': {
            ...
            if (screen === 'review' && !state.extraction || screen === 'plan' && !state.result)
                return;
            state.screen = screen;
            state.error = '';
            render();
        ...
        case 'sample':
            ...
            break;
        case 'extract':
            if (!state.file || !state.consent)
                return;
            await run('Understanding your requirements…', async () => {
                const body = new FormData();
                body.append('file', state.file);
                body.append('consent', 'true');
                body.append('implementation_context', state.context);
                body.append('vision_mode', state.visionMode);
                state.extraction = await api('/api/extract', { method: 'POST', body });
                state.result = null;
                state.sample = false;
                state.screen = 'review';
                state.corrections = {};
            ...
            break;
        case 'generate':
            if (!state.extraction || !state.acknowledged || state.sample)
                return;
            if (Object.values(state.corrections).some(v => !v.trim())) {
                state.error = 'A requirement description cannot be empty. Correct it rather than deleting its scope.';
                render();
                return;
            }
            await run('Turning the spec into a build plan…', async () => {
                state.result = await api('/api/plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ extraction: state.extraction, review: collectReview() }) });
                state.screen = 'plan';
                state.tab = 'tasks';
                state.selectedTask = state.result.plan.tasks[0].id;
            });
            break;
        case 'tab':
            ...
            break;
        case 'task':
            state.selectedTask = element.dataset.id;
            state.tab = 'tasks';
            render();
            break;
        case 'source':
            showSource(Number(element.dataset.page));
            break;
        case 'close-source':
            sourceDialog.close();
            break;
        case 'copy-prompt': {
            ...
            try {
                ...
                toast(`Copied ${task.id} agent prompt`);
            }
            catch {
                toast('Clipboard access was blocked. Expand the prompt and copy its text manually.');
            }
            break;
        }
        case 'export-json':
            if (state.result)
                download(JSON.stringify(state.result, null, 2), 'engineering-plan.json', 'application/json');
            break;
        case 'export-md':
            if (state.result)
                download(state.result.markdown, 'engineering-plan.md', 'text/markdown;charset=utf-8');
            break;
        case 'export-prompts':
            if (state.result)
                download(`# Coding-agent prompts\n\nMode: ${state.result.mode}\n\n` + state.result.plan.tasks.map(t => `## ${t.order}. ${t.id} — ${t.title}\n\n${t.agent_prompt}\n`).join('\n---\n\n'), 'agent-prompts.md', 'text/markdown;charset=utf-8');
            break;
    }
}
...
document.addEventListener('input', event => {
    ...
    if (input.id === 'context' || input.id === 'review-context')
    ...
    if (!state.sample && state.result && (input.id === 'review-context' || input.dataset.requirement || input.dataset.question)) {
        state.result = null;
        document.querySelector('.step[data-screen="plan"]')?.setAttribute('disabled', '');
    }
    if (input.dataset.requirement)
        state.corrections[input.dataset.requirement] = input.value;
    if (input.dataset.question)
        state.answers[input.dataset.question] = input.value;
    if (input.id === 'task-search') {
        const query = input.value.trim().toLowerCase();
        document.querySelectorAll('.task-item').forEach(item => { item.hidden = !item.dataset.search?.includes(query); });
    }
});
document.addEventListener('change', event => {
    const input = event.target;
    if (input.id === 'pdf-file' && input.files?.[0])
        selectFile(input.files[0]);
    if (input.id === 'reading-mode')
        state.visionMode = input.value;
    if (input.id === 'consent') {
        state.consent = input.checked;
        const button = document.querySelector('#extract-button');
        if (button)
            button.disabled = !state.file || !state.consent || !state.health?.azure_configured;
    }
    if (input.id === 'acknowledge') {
        state.acknowledged = input.checked;
        document.querySelector('#generate-button').disabled = !state.acknowledged;
    }
});
...
document.addEventListener('drop', event => {
    ...
    if (zone && !state.busy && event.dataTransfer?.files[0])
        selectFile(event.dataTransfer.files[0]);
});
sourceDialog.addEventListener('click', event => { if (event.target === sourceDialog)
    sourceDialog.close(); });
...
void api('/api/health').then(health => { state.health = health; render(); }).catch(error => { state.error = `The backend is unavailable. ${error instanceof Error ? error.message : ''}`; render(); });
export {};
